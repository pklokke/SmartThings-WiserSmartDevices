...
metadata {
	definition (name: "Wiser Radiator Thermostat", namespace: "pklokke", author: "pklokke", mnmn: "SmartThings", vid: "generic-thermostat-1", genericHandler: "Zigbee") {
		capability "Actuator"
...
def configure() {
    ...
    if (isVACT()) {
    	binding += zigbee.addBinding(POWER_CONFIGURATION_CLUSTER)
    }
	
    def configCmds = zigbee.batteryConfig() +
		zigbee.configureReporting(THERMOSTAT_CLUSTER, LOCAL_TEMPERATURE, DataType.INT16, 30, 3600, 0x0064) +
        zigbee.configureReporting(THERMOSTAT_CLUSTER, HEATING_SETPOINT, DataType.INT16, 600, 600, 0x0064)
